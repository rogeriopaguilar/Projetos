/**
   Adaptada por Rogério de Paula Aguilar em out/2007
   para funcionar em conjunto com a api sarissa
 ...
 * Alteração --> context deve ser um objeto texto com o xml
 */
XML.XPathExpression.prototype.getNodes = function(context) {
    
    //Alteração RAGUILAR 01
    if(!(_SARISSA_IS_IE) && (typeof context) == 'string')
    {
		context = XML.parse(context);    
    }
	//FINAL ALTERAÇÃO RAGUILAR 01
    ...
    if (this.xpathExpr) {
        ...
        try {
    		...
    		//Alteração RAGUILAR 02
			try
			{
	            /*
	            	Tenta fazer a gambiarra para funcionar no IE

	            */
	            if(!( (typeof context) == 'string'))
	            {
	            	throw new Error('Para a gambi do IE é necessário passar o xml como texto!');
	            }
				var xml = context;
				var rnd = parseInt(Math.random() * 99999);
				var idElemento = "DNSEC_txtTextXPath" + rnd;
				if(_SARISSA_IS_IE)
				{
					var txtTextPath = document.createElement('<textarea id="' + idElemento + '"></textarea>');				
				}else
				{
					throw new Error('Função apenas para o IE!');
				}
				txtTextPath.value = xml;
				var xmlStr = txtTextPath.value.replace(/^\s+/, "").replace(/\s+$/, "");
				var x = new DOMParser().parseFromString(xmlStr, "text/xml");
				var result = x.selectNodes(this.xpathText);
...
XML.returnNodesValues = function(xml, nodeName)
{
	var arrayNodes = new Array();
	var contNodes = 0;
	for(var cont = 0; cont < xml.documentElement.childNodes.length; cont++)
	{
		if(xml.documentElement.childNodes[cont].nodeName == nodeName)
		{
			arrayNodes[contNodes] = xml.documentElement.childNodes[cont];
			contNodes++;
		}
	}
	for(cont = 0; cont < contNodes; cont++)
	{
		alert(arrayNodes[cont]);
	}
	return arrayNodes;
